  ...
  if (isPadre) {
    return (
      <div className="animate-fade-in">
        <h1 className="page-title mb-6">Reportes Semanales - Colegio Harvard</h1>
        {hijos.length === 0 ? (
          <Card><p className="text-center text-gold-600 py-8">No se encontraron hijos vinculados</p></Card>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {hijos.map(hijo => (
              <button key={hijo.id} onClick={() => navigate(`/reportes-semanales/alumno/${hijo.id}`)} className="text-left group">
      ...
      {aulasFiltradas.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {aulasFiltradas.map(aula => {
            ...
            return (
              <button key={aula.id} onClick={() => navigate(`/reportes-semanales/aula/${aula.id}`)} className="text-left group">
                <Card className="hover:shadow-lg hover:border-gold-300 transition-all duration-200 h-full">
