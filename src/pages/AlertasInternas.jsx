...
export default function AlertasInternas() {
  ...
  return <div className="space-y-5">
    ...
    {editando && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4"><div className="w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl"><h2 className="font-display text-2xl font-bold text-primary-800">{editando.nueva ? 'Nueva alerta interna' : editando.estado === 'RESUELTA' ? 'Reactivar alerta' : 'Editar alerta'}</h2><div className="mt-5 space-y-4">
      <div><label className="mb-1 block text-sm font-semibold text-primary-800">Alumno</label>{editando.nueva ? <><input value={busquedaAlumno} onChange={e => { setBusquedaAlumno(e.target.value); setAlumnoId(''); }} className="input-field" placeholder="Escriba código, DNI o nombre"/><div className="mt-2 max-h-44 overflow-y-auto rounded-lg border border-cream-200">{alumnosFiltrados.map(a => <button key={a.id} type="button" onClick={() => { setAlumnoId(String(a.id)); setBusquedaAlumno(`${a.codigo_alumno} · ${a.nombre_completo}`); }} className={`block w-full border-b border-cream-100 px-3 py-2 text-left text-sm hover:bg-cream-50 ${String(a.id) === alumnoId ? 'bg-gold-50 font-semibold' : ''}`}>{a.codigo_alumno} · {a.nombre_completo}<span className="block text-xs text-primary-600">{aulaDe({ tbl_aulas: a.aula ? { seccion: a.aula.seccion, tbl_grados: { nombre: a.aula.grado?.nombre, tbl_niveles: { nombre: a.aula.grado?.nivel } } } : null })}</span></button>)}</div></> : <div className="rounded-lg bg-cream-50 p-3 font-semibold text-primary-800">{busquedaAlumno}</div>}</div>
