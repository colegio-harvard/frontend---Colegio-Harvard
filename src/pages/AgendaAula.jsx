...
import { HiArrowLeft, HiAcademicCap, HiUser, HiUserGroup, HiChevronRight, HiBookOpen } from 'react-icons/hi';
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate('/agenda')} className="p-2 text-primary-800/70 hover:bg-cream-100 rounded-lg transition-colors">
      ...
      {alumnos.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {alumnos.map(alumno => (
            <button
              key={alumno.id}
              onClick={() => navigate(`/agenda/alumno/${alumno.id}`)}
              ...
              <Card className="hover:shadow-lg hover:border-gold-300 transition-all duration-200 h-full">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1">
                    <HiBookOpen className="w-4 h-4 text-cream-300 group-hover:text-gold-500 transition-colors" />
