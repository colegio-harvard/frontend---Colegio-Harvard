...
import { HiCheckCircle, HiClock, HiCog, HiEye, HiEyeOff, HiMail, HiMinusCircle, HiPlus, HiPrinter, HiRefresh, HiSearch, HiUserAdd, HiX } from 'react-icons/hi';
import Card from '../components/ui/Card';
import Modal from '../components/ui/Modal';
import CampoOpciones from '../components/ui/CampoOpciones';
import { cargarMatriculas, generarInvitacionMatricula, guardarBorradorAsistidoMatricula, guardarComplementoAdministrativoMatricula, guardarConfiguracionMatricula, guardarControlDocumentalMatricula, obtenerExpedienteMatricula, prepararMatricula, revisarMatricula } from '../services/matriculasService';
...
const documentStatusClasses = {
  ENTREGADO: { row: 'border-emerald-200 bg-emerald-50/60', select: 'border-emerald-300 bg-emerald-50 text-emerald-800', icon: HiCheckCircle, iconClass: 'text-emerald-600' },
  PENDIENTE: { row: 'border-amber-200 bg-amber-50/50', select: 'border-amber-300 bg-amber-50 text-amber-800', icon: HiClock, iconClass: 'text-amber-600' },
  NO_APLICA: { row: 'border-gray-200 bg-gray-50', select: 'border-gray-300 bg-gray-50 text-gray-600', icon: HiMinusCircle, iconClass: 'text-gray-500' },
};
...
export default function Matriculas() {
  ...
  const [adminComplementEditing, setAdminComplementEditing] = useState(false);
  const [adminComplementReason, setAdminComplementReason] = useState('');
  const [currentAcademic, setCurrentAcademic] = useState({ id_nivel: '', id_grado: '', id_aula: '' });
  const [newStudentOpen, setNewStudentOpen] = useState(false);
  const [studentSaving, setStudentSaving] = useState(false);
  const [studentSyncing, setStudentSyncing] = useState(false);
  const [aulas, setAulas] = useState([]);
  const [niveles, setNiveles] = useState([]);
  const [studentForm, setStudentForm] = useState({ codigo_alumno: '', dni: '', apellido_paterno: '', apellido_materno: '', nombres: '', monto_matricula: '', monto_materiales: '', monto_pension: '', id_nivel: '', id_grado: '', id_aula: '' });
  const [parentMode, setParentMode] = useState('existing');
  const [parentQuery, setParentQuery] = useState('');
  const [parentResults, setParentResults] = useState([]);
  const [selectedParent, setSelectedParent] = useState(null);
  const [parentForm, setParentForm] = useState({ dni: '', apellido_paterno: '', apellido_materno: '', nombres: '', celular: '', username: '', contrasena: '' });
  ...
  const currentGrades = useMemo(() => niveles.find((n) => n.id === Number(currentAcademic.id_nivel))?.grados?.filter((g) => aulas.some((a) => a.id_grado === g.id)) || [], [niveles, aulas, currentAcademic.id_nivel]);
  const currentClassrooms = useMemo(() => aulas.filter((a) => a.id_grado === Number(currentAcademic.id_grado)), [aulas, currentAcademic.id_grado]);
  ...
  const openNewStudent = async () => {
    try {
      ...
      setStudentForm({ codigo_alumno: codigo, dni: '', apellido_paterno: '', apellido_materno: '', nombres: '', monto_matricula: '', monto_materiales: '', monto_pension: '', id_nivel: '', id_grado: '', id_aula: '' });
      setParentMode('existing'); setParentQuery(''); setParentResults([]); setSelectedParent(null);
      setParentForm({ dni: '', apellido_paterno: '', apellido_materno: '', nombres: '', celular: '', username: '', contrasena: '' });
  ...
  const saveConfig = async (event) => {
    ...
    try {
      ...
      await guardarConfiguracionMatricula({ fecha_inicio: form.get('fecha_inicio') || null, fecha_fin: form.get('fecha_fin') || null, nombre_documentos: form.get('nombre_documentos'), enlace_documentos: form.get('enlace_documentos'), enlace_descarga_app: form.get('enlace_descarga_app'), version_documentos: form.get('version_documentos'), activo: form.get('activo') === 'on', documentos_json: data.config.documentos_json });
  ...
  const openDetail = async (id) => {
    try { const response = await obtenerExpedienteMatricula(id); const expediente = response.data.data; const formularioBase = Object.keys(expediente.borrador_asistido || {}).length ? expediente.borrador_asistido : expediente.datos_formulario; const traslado = formularioBase?.tipo_ingreso === 'TRASLADO'; const defaults = Object.fromEntries(documentosControl.map(([key]) => [key, { estado: ['certificado_estudios', 'libreta_anterior'].includes(key) && !traslado ? 'NO_APLICA' : 'PENDIENTE', observacion: '' }])); const nombreGuardado = expediente.datos_snapshot?.alumno || {}; const nombreInferido = separarNombreRegistrado(nombreGuardado.nombre); const representanteGuardado = expediente.datos_snapshot?.apoderado || {}; const representanteInferido = separarNombreRegistrado(representanteGuardado.nombre); setDetail(expediente); setAssistedDraft({ ...emptyAssistedDraft, ...(formularioBase || {}), alumno_apellido_paterno: formularioBase?.alumno_apellido_paterno || nombreGuardado.apellido_paterno || nombreInferido.apellido_paterno, alumno_apellido_materno: formularioBase?.alumno_apellido_materno || nombreGuardado.apellido_materno || nombreInferido.apellido_materno, alumno_nombres: formularioBase?.alumno_nombres || nombreGuardado.nombres || nombreInferido.nombres, alumno_dni: formularioBase?.alumno_dni || nombreGuardado.dni || '', representante_apellido_paterno: formularioBase?.representante_apellido_paterno || representanteGuardado.apellido_paterno || representanteInferido.apellido_paterno, representante_apellido_materno: formularioBase?.representante_apellido_materno || representanteGuardado.apellido_materno || representanteInferido.apellido_materno, representante_nombres: formularioBase?.representante_nombres || representanteGuardado.nombres || representanteInferido.nombres, representante_dni: formularioBase?.representante_dni || representanteGuardado.dni || '', celular: formularioBase?.celular || representanteGuardado.celular || '', persona_autorizada_1: { ...emptyAssistedDraft.persona_autorizada_1, ...(formularioBase?.persona_autorizada_1 || {}) }, persona_autorizada_2: { ...emptyAssistedDraft.persona_autorizada_2, ...(formularioBase?.persona_autorizada_2 || {}) } }); setAssistedEditing(false); setRepresentanteRetiraAsistido(false); setDocumentCheck({ ...defaults, ...(expediente.control_documental || {}) }); setObservation(expediente.observacion_revision || ''); }
  ...
  const saveAssistedDraft = async () => {
    ...
    try {
      if (['ACEPTADA', 'COMPLETADA'].includes(detail.estado)) {
        if (adminComplementReason.trim().length < 10) return toast.error('Indique una justificación de por lo menos 10 caracteres');
        if (!currentAcademic.id_aula) return toast.error('Seleccione el nivel, grado y sección actuales');
        await guardarComplementoAdministrativoMatricula(detail.id, assistedDraft, adminComplementReason, currentAcademic.id_aula);
        setAssistedEditing(false); setAdminComplementEditing(false); setAdminComplementReason(''); await load(); await openDetail(detail.id);
        toast.success('Complemento administrativo guardado sin modificar la evidencia original');
      } else {
        const response = await guardarBorradorAsistidoMatricula(detail.id, assistedDraft);
        setDetail({ ...detail, estado: response.data.data.estado || detail.estado, observacion_revision: response.data.data.estado === 'ABIERTA' ? null : detail.observacion_revision, borrador_asistido: response.data.data.borrador_asistido, borrador_preparado_en: response.data.data.borrador_preparado_en });
        setAssistedEditing(false); await load(); toast.success('Datos oficiales guardados. Ya puede generar la invitación.');
      }
    }
    catch (error) { toast.error(error.response?.data?.error || 'No se pudo guardar el borrador asistido'); }
    finally { setAssistedSaving(false); }
  };
  const changeEntryType = (tipoIngreso) => {
    setAssistedDraft((actual) => {
      if (tipoIngreso === 'PROMOCION_INTERNA') return { ...actual, tipo_ingreso: tipoIngreso, institucion_procedencia: 'Colegio Harvard' };
      if (tipoIngreso === 'INGRESO_INICIAL') return { ...actual, tipo_ingreso: tipoIngreso, condicion_promocion: '', anio_escolar_anterior: '', nivel_anterior: '', grado_anterior: '', institucion_procedencia: '', codigo_modular_procedencia: '', ubicacion_procedencia: '' };
      if (tipoIngreso === 'TRASLADO' && actual.tipo_ingreso === 'PROMOCION_INTERNA') return { ...actual, tipo_ingreso: tipoIngreso, institucion_procedencia: '', codigo_modular_procedencia: '', ubicacion_procedencia: '' };
      return { ...actual, tipo_ingreso: tipoIngreso };
    });
  };
  const toggleAdminComplement = async () => {
    const editing = !adminComplementEditing;
    if (!editing) { setAdminComplementEditing(false); setAssistedEditing(false); setAdminComplementReason(''); return; }
    try {
      const [aulasResponse, nivelesResponse] = await Promise.all([listarAulas(), listarNiveles()]);
      setAulas(aulasResponse.data.data || []); setNiveles(nivelesResponse.data.data || []);
      setCurrentAcademic({ id_nivel: String(detail.id_nivel_actual || ''), id_grado: String(detail.id_grado_actual || ''), id_aula: String(detail.id_aula_actual || '') });
      setAssistedDraft({ ...emptyAssistedDraft, ...(detail.datos_formulario || {}), ...(detail.complemento_administrativo || {}) });
      setAdminComplementEditing(true); setAssistedEditing(true);
    } catch (error) { toast.error(error.response?.data?.error || 'No se pudo cargar la configuración académica'); }
  };
  ...
  const syncStudentRecord = async () => {
    const form = detail?.datos_formulario || {};
    const nombreCompleto = [form.alumno_nombres, form.alumno_apellido_paterno, form.alumno_apellido_materno].filter(Boolean).join(' ').trim();
  ...
  const detailData = detail?.aceptado_en ? { ...(detail.datos_formulario || {}), ...(detail.complemento_administrativo || {}) } : (Object.keys(detail?.borrador_asistido || {}).length ? detail.borrador_asistido : (detail?.datos_formulario || {}));
  ...
  return <div className="space-y-6 pb-10">
    ...
    {showConfig && <Card title={`Configuración ${data.anio.anio}`}>
      <form onSubmit={saveConfig} className="grid gap-4 md:grid-cols-2">
        ...
        <label className="text-sm text-primary-700 md:col-span-2">Enlace de descarga de la app<input name="enlace_descarga_app" type="url" placeholder="https://play.google.com/... o https://..." defaultValue={config.enlace_descarga_app || ''} className={`${field} mt-1`} /><small className="mt-1 block text-primary-400">Opcional. Se añadirá automáticamente al mensaje de invitación de matrícula.</small></label>
    ...
    <Modal isOpen={newStudentOpen} onClose={() => setNewStudentOpen(false)} title="Nuevo alumno para matrícula digital" size="lg">
      <form onSubmit={saveNewStudent} className="space-y-5">
        <div><h3 className="mb-3 font-semibold text-primary-800">Datos del estudiante</h3><div className="grid gap-3 md:grid-cols-2">
          ...
          <label className="text-sm">Apellido paterno *<input required value={studentForm.apellido_paterno} onChange={(e) => setStudentForm({...studentForm,apellido_paterno:e.target.value})} className={`${field} mt-1`}/></label>
          <label className="text-sm">Apellido materno *<input required value={studentForm.apellido_materno} onChange={(e) => setStudentForm({...studentForm,apellido_materno:e.target.value})} className={`${field} mt-1`}/></label>
          <label className="text-sm md:col-span-2">Nombres *<input required value={studentForm.nombres} onChange={(e) => setStudentForm({...studentForm,nombres:e.target.value})} className={`${field} mt-1`}/></label>
        ...
        <div><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><h3 className="font-semibold text-primary-800">Padre / apoderado</h3><div className="flex rounded-lg border p-1"><button type="button" onClick={()=>{setParentMode('existing');setSelectedParent(null);}} className={`rounded-md px-3 py-1.5 text-sm ${parentMode==='existing'?'bg-primary-700 text-white':''}`}>Existente</button><button type="button" onClick={()=>setParentMode('new')} className={`rounded-md px-3 py-1.5 text-sm ${parentMode==='new'?'bg-primary-700 text-white':''}`}><HiUserAdd className="inline"/> Nuevo</button></div></div>
          {parentMode === 'existing' ? <div>{selectedParent ? <div className="flex items-center justify-between rounded-lg border border-emerald-200 bg-emerald-50 p-3"><div><b>{selectedParent.nombre_completo}</b><p className="text-sm">DNI {selectedParent.dni} · {selectedParent.celular}</p></div><button type="button" onClick={()=>setSelectedParent(null)} className="text-sm text-primary-700">Cambiar</button></div> : <><div className="flex gap-2"><input value={parentQuery} onChange={(e)=>setParentQuery(e.target.value)} placeholder="DNI o nombre del apoderado" className={field}/><button type="button" onClick={searchParent} className={`${btn} border`}><HiSearch/>Buscar</button></div>{parentResults.length > 0 && <div className="mt-2 max-h-40 overflow-y-auto rounded-lg border">{parentResults.map((p)=><button type="button" key={p.id} onClick={()=>setSelectedParent(p)} className="block w-full border-b p-3 text-left hover:bg-cream-50"><b>{p.nombre_completo}</b> · DNI {p.dni} · {p.celular}</button>)}</div>}</>}</div> : <div className="grid gap-3 md:grid-cols-2">
            <label className="text-sm">DNI *<input required inputMode="numeric" maxLength="8" value={parentForm.dni} onChange={(e)=>setParentForm({...parentForm,dni:e.target.value.replace(/\D/g,'').slice(0,8)})} className={`${field} mt-1`}/></label><label className="text-sm">Apellido paterno *<input required value={parentForm.apellido_paterno} onChange={(e)=>setParentForm({...parentForm,apellido_paterno:e.target.value})} className={`${field} mt-1`}/></label><label className="text-sm">Apellido materno<input value={parentForm.apellido_materno} onChange={(e)=>setParentForm({...parentForm,apellido_materno:e.target.value})} className={`${field} mt-1`}/></label><label className="text-sm">Nombres *<input required value={parentForm.nombres} onChange={(e)=>setParentForm({...parentForm,nombres:e.target.value})} className={`${field} mt-1`}/></label><label className="text-sm">Celular *<input required inputMode="numeric" maxLength="9" value={parentForm.celular} onChange={(e)=>setParentForm({...parentForm,celular:e.target.value.replace(/\D/g,'').slice(0,9)})} className={`${field} mt-1`}/></label><label className="text-sm">Usuario de acceso *<input required value={parentForm.username} onChange={(e)=>setParentForm({...parentForm,username:e.target.value})} className={`${field} mt-1`}/></label><label className="text-sm md:col-span-2">Contraseña inicial *<span className="relative mt-1 block"><input required type={showParentPassword ? 'text' : 'password'} value={parentForm.contrasena} onChange={(e)=>setParentForm({...parentForm,contrasena:e.target.value})} className={`${field} pr-12`}/><button type="button" onClick={()=>setShowParentPassword(!showParentPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 rounded p-1 text-primary-500 hover:bg-cream-100 hover:text-primary-800" aria-label={showParentPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'} title={showParentPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'}>{showParentPassword ? <HiEyeOff className="h-5 w-5"/> : <HiEye className="h-5 w-5"/>}</button></span></label>
          </div>}
        </div>
    ...
    {['ACEPTADA','COMPLETADA'].includes(detail?.estado) && usuario?.rol_codigo === 'SUPER_ADMIN' && <div className="fixed bottom-5 right-5 z-[80] max-w-sm rounded-xl border border-blue-300 bg-blue-50 p-4 shadow-xl print:hidden"><p className="text-sm font-semibold text-blue-900">Corrección administrativa</p><p className="mt-1 text-xs text-blue-800">Permite corregir datos sin alterar la aceptación ni la huella de evidencia original.</p>{adminComplementEditing && <textarea value={adminComplementReason} onChange={(e) => setAdminComplementReason(e.target.value)} placeholder="Justificación obligatoria (mínimo 10 caracteres)" maxLength="1000" className={`${field} mt-3 bg-white text-sm`}/>}<button type="button" onClick={toggleAdminComplement} className={`${btn} mt-3 w-full bg-blue-700 text-white`}>{adminComplementEditing ? 'Cerrar complemento' : (detail.complementado_en ? 'Editar corrección administrativa' : 'Corregir datos de matrícula')}</button>{detail.complementado_en && !adminComplementEditing && <p className="mt-2 text-xs text-blue-700">Última actualización: {new Date(detail.complementado_en).toLocaleString('es-PE')}<br/>Motivo: {detail.motivo_complemento}</p>}</div>}

    {detail && <div className="fixed inset-0 z-[70] overflow-y-auto bg-primary-950/50 p-4"><div className="mx-auto my-6 max-w-4xl rounded-2xl bg-white p-6 shadow-2xl print:my-0 print:shadow-none"><div className="flex justify-between"><div><h2 className="font-display text-2xl text-primary-800">Expediente {detail.codigo}</h2><p className="text-sm text-primary-500">{detail.datos_snapshot?.alumno?.nombre} · {detail.estado}</p><p className="mt-1 text-xs font-semibold text-primary-600">Código del alumno: {detail.datos_snapshot?.alumno?.codigo || 'No registrado'}</p></div><button className="print:hidden" onClick={() => setDetail(null)}><HiX className="h-6 w-6"/></button></div><div className="mt-5 grid gap-4 md:grid-cols-3">{[['Apoderado', `${detail.datos_snapshot?.apoderado?.nombre || ''}${detailData?.vinculo_representante ? ` · ${detailData.vinculo_representante}` : ''}`], ['Aceptado', detail.aceptado_en ? new Date(detail.aceptado_en).toLocaleString('es-PE') : 'Pendiente'], ['Información económica', <span className="block space-y-1 text-sm"><span className="block"><b>Deuda:</b> {money(detail.deuda_snapshot)}</span><span className="block"><b>Matrícula:</b> {money(detail.costo_matricula_snapshot ?? detail.montos_ficha_actual?.matricula)}</span><span className="block"><b>Mensualidad:</b> {money(detail.datos_snapshot?.alumno?.monto_pension ?? detail.montos_ficha_actual?.pension)}</span><span className="block"><b>Materiales:</b> {money(detail.datos_snapshot?.alumno?.monto_materiales ?? detail.montos_ficha_actual?.materiales)}</span></span>]].map(([a,b]) => <div key={a} className="rounded-xl bg-cream-50 p-4"><p className="text-xs uppercase text-primary-500">{a}</p><p className="mt-1 font-semibold">{b}</p></div>)}</div><div className="mt-6 rounded-xl border-2 border-gold-200 bg-amber-50/40 p-4 print:hidden"><div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="font-semibold text-primary-800">Matrícula asistida</h3><p className="text-sm text-primary-500">{detail.borrador_preparado_en ? `Borrador preparado el ${new Date(detail.borrador_preparado_en).toLocaleString('es-PE')}` : 'Prepare los datos para que el apoderado solamente los revise y confirme.'}</p></div>{!['ACEPTADA','COMPLETADA'].includes(detail.estado) && <button type="button" onClick={() => setAssistedEditing(!assistedEditing)} className={`${btn} border bg-white`}>{assistedEditing ? 'Cerrar edición' : 'Preparar datos'}</button>}</div>{assistedEditing && <div className="mt-4 space-y-4"><div><h4 className="font-semibold text-primary-800">Datos del/de la estudiante</h4><p className="mt-1 text-xs text-primary-500">Las correcciones quedan en esta matrícula y no cambian automáticamente su ficha principal.</p>{detail.datos_snapshot?.alumno?.nombre && <p className="mt-2 rounded-lg bg-cream-50 p-2 text-xs"><b>Nombre registrado:</b> {detail.datos_snapshot.alumno.nombre}</p>}<div className="mt-3 grid gap-3 md:grid-cols-2"><label className="text-sm">Apellido paterno<input value={assistedDraft.alumno_apellido_paterno || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, alumno_apellido_paterno: e.target.value })} className={`${field} mt-1`} /></label><label className="text-sm">Apellido materno<input value={assistedDraft.alumno_apellido_materno || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, alumno_apellido_materno: e.target.value })} className={`${field} mt-1`} /></label><label className="text-sm">Nombres<input value={assistedDraft.alumno_nombres || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, alumno_nombres: e.target.value })} className={`${field} mt-1`} /></label><label className="text-sm">DNI<input inputMode="numeric" maxLength="8" value={assistedDraft.alumno_dni || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, alumno_dni: e.target.value.replace(/\D/g, '').slice(0, 8) })} className={`${field} mt-1`} /></label><label className="text-sm">Fecha de nacimiento<input type="date" value={assistedDraft.alumno_fecha_nacimiento || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, alumno_fecha_nacimiento: e.target.value })} className={`${field} mt-1`} /></label><label className="text-sm">Sexo<select value={assistedDraft.alumno_sexo || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, alumno_sexo: e.target.value })} className={`${field} mt-1`}><option value="">Seleccione...</option><option value="FEMENINO">Femenino</option><option value="MASCULINO">Masculino</option></select></label><label className="text-sm">País de origen<input value={assistedDraft.alumno_pais_origen || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, alumno_pais_origen: e.target.value })} className={`${field} mt-1`} /></label><label className="text-sm">Departamento / provincia / distrito<input value={assistedDraft.alumno_ubicacion_origen || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, alumno_ubicacion_origen: e.target.value })} className={`${field} mt-1`} /></label><label className="text-sm">¿Requiere cuidado especial?<select value={assistedDraft.alumno_requiere_cuidado_especial || 'NO'} onChange={(e) => setAssistedDraft({ ...assistedDraft, alumno_requiere_cuidado_especial: e.target.value, alumno_detalle_cuidado_especial: e.target.value === 'SI' ? assistedDraft.alumno_detalle_cuidado_especial : '' })} className={`${field} mt-1`}><option value="NO">No</option><option value="SI">Sí</option></select></label>{assistedDraft.alumno_requiere_cuidado_especial === 'SI' && <label className="text-sm">Detalle del cuidado especial<input value={assistedDraft.alumno_detalle_cuidado_especial || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, alumno_detalle_cuidado_especial: e.target.value })} className={`${field} mt-1`} /></label>}</div></div><div className="border-t pt-4"><h4 className="font-semibold text-primary-800">Datos del representante legal</h4><p className="mt-1 text-xs text-primary-500">Las correcciones quedan en la ficha de matrícula y no cambian su cuenta de acceso.</p><div className="mt-3 grid gap-3 md:grid-cols-2"><label className="text-sm">Apellido paterno<input value={assistedDraft.representante_apellido_paterno || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, representante_apellido_paterno: e.target.value })} className={`${field} mt-1`} /></label><label className="text-sm">Apellido materno<input value={assistedDraft.representante_apellido_materno || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, representante_apellido_materno: e.target.value })} className={`${field} mt-1`} /></label><label className="text-sm">Nombres<input value={assistedDraft.representante_nombres || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, representante_nombres: e.target.value })} className={`${field} mt-1`} /></label><label className="text-sm">DNI<input inputMode="numeric" maxLength="8" value={assistedDraft.representante_dni || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, representante_dni: e.target.value.replace(/\D/g, '').slice(0, 8) })} className={`${field} mt-1`} /></label>{[['vinculo_representante','Vínculo con el estudiante'],['celular','Celular'],['email','Correo electrónico'],['direccion','Dirección actual']].map(([key,label]) => key === 'vinculo_representante' ? <CampoOpciones key={key} label={label} value={assistedDraft[key] || ''} onChange={(value) => setAssistedDraft({ ...assistedDraft, [key]: value })} options={['Padre','Madre','Hermano(a)','Tío','Abuelo','Tutor legal']}/> : <label key={key} className="text-sm">{label}<input value={assistedDraft[key] || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, [key]: e.target.value })} className={`${field} mt-1`} /></label>)}</div></div><div className="border-t pt-4"><h4 className="font-semibold text-primary-800">Datos de emergencia</h4><div className="mt-3 grid gap-3 md:grid-cols-2">{[['contacto_emergencia','Contacto de emergencia'],['telefono_emergencia','Teléfono de emergencia'],['centro_salud_emergencia','Centro o cobertura de salud'],['observaciones_salud','Observaciones de salud']].map(([key,label]) => key === 'centro_salud_emergencia' ? <CampoOpciones key={key} label={label} value={assistedDraft[key] || ''} onChange={(value) => setAssistedDraft({ ...assistedDraft, [key]: value })} options={['Posta de salud','EsSalud','SIS','Seguro particular']}/> : <label key={key} className="text-sm">{label}<input value={assistedDraft[key] || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, [key]: e.target.value })} className={`${field} mt-1`} /></label>)}</div></div><div className="border-t pt-4"><h4 className="font-semibold text-primary-800">Información académica de la matrícula</h4><p className="mt-1 text-xs text-primary-500">Registre la situación académica del año anterior y la procedencia del estudiante para sustentar su matrícula actual.</p><div className="mt-3 grid gap-3 md:grid-cols-2"><label className="text-sm">Tipo de ingreso<select value={assistedDraft.tipo_ingreso} onChange={(e) => changeEntryType(e.target.value)} className={`${field} mt-1`}><option value="">Seleccione...</option><option value="PROMOCION_INTERNA">Promoción interna</option><option value="TRASLADO">Traslado</option><option value="INGRESO_INICIAL">Ingreso inicial</option><option value="REPITENCIA">Repitencia</option></select></label><label className="text-sm">Condición de promoción<select value={assistedDraft.condicion_promocion} onChange={(e) => setAssistedDraft({ ...assistedDraft, condicion_promocion: e.target.value })} className={`${field} mt-1`}><option value="">Seleccione...</option><option value="PROMOVIDO">Promovido</option><option value="REPITE">Repite el grado</option><option value="PENDIENTE">Pendiente de documentos</option><option value="UBICACION">Ubicación</option></select></label><label className="text-sm">Año escolar anterior<input disabled={assistedDraft.tipo_ingreso === 'INGRESO_INICIAL'} value={assistedDraft.anio_escolar_anterior || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, anio_escolar_anterior: e.target.value.replace(/\D/g, '').slice(0, 4) })} inputMode="numeric" maxLength="4" className={`${field} mt-1`} /></label><label className="text-sm">Nivel anterior<select disabled={assistedDraft.tipo_ingreso === 'INGRESO_INICIAL'} value={assistedDraft.nivel_anterior || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, nivel_anterior: e.target.value, grado_anterior: '' })} className={`${field} mt-1`}><option value="">Seleccione...</option><option>Inicial</option><option>Primaria</option><option>Secundaria</option></select></label><label className="text-sm">Grado anterior<select disabled={!assistedDraft.nivel_anterior || assistedDraft.tipo_ingreso === 'INGRESO_INICIAL'} value={assistedDraft.grado_anterior || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, grado_anterior: e.target.value })} className={`${field} mt-1 disabled:bg-cream-100`}><option value="">{assistedDraft.nivel_anterior ? 'Seleccione el grado...' : 'Seleccione primero el nivel...'}</option>{(gradosPorNivel[assistedDraft.nivel_anterior] || []).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="text-sm">Nivel actual<select value={currentAcademic.id_nivel} onChange={(e) => setCurrentAcademic({ id_nivel: e.target.value, id_grado: '', id_aula: '' })} className={`${field} mt-1`}><option value="">Seleccione...</option>{niveles.map((nivel) => <option key={nivel.id} value={nivel.id}>{nivel.nombre}</option>)}</select></label><label className="text-sm">Grado actual<select disabled={!currentAcademic.id_nivel} value={currentAcademic.id_grado} onChange={(e) => setCurrentAcademic({ ...currentAcademic, id_grado: e.target.value, id_aula: '' })} className={`${field} mt-1 disabled:bg-cream-100`}><option value="">Seleccione...</option>{currentGrades.map((grado) => <option key={grado.id} value={grado.id}>{grado.nombre}</option>)}</select></label><label className="text-sm">Sección actual<select disabled={!currentAcademic.id_grado} value={currentAcademic.id_aula} onChange={(e) => setCurrentAcademic({ ...currentAcademic, id_aula: e.target.value })} className={`${field} mt-1 disabled:bg-cream-100`}><option value="">Seleccione...</option>{currentClassrooms.map((aula) => <option key={aula.id} value={aula.id}>{aula.seccion}</option>)}</select></label>{[['institucion_procedencia','Institución de procedencia'],['codigo_modular_procedencia','Código modular'],['ubicacion_procedencia','Departamento / provincia / distrito']].map(([key,label]) => <label key={key} className="text-sm">{label}<input disabled={assistedDraft.tipo_ingreso === 'INGRESO_INICIAL' || (assistedDraft.tipo_ingreso === 'PROMOCION_INTERNA' && key === 'institucion_procedencia')} value={assistedDraft[key] || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, [key]: e.target.value })} className={`${field} mt-1 disabled:bg-cream-100`} /></label>)}</div></div><div className="border-t pt-4"><h4 className="font-semibold text-primary-800">Personas autorizadas para recoger al estudiante</h4><p className="mt-1 text-xs text-primary-500">La persona principal es obligatoria; la alternativa es opcional.</p><label className="mt-3 flex cursor-pointer items-center gap-2 rounded-lg border border-gold-200 bg-amber-50 p-3 text-sm"><input type="checkbox" checked={representanteRetiraAsistido} onChange={(e) => { const checked = e.target.checked; setRepresentanteRetiraAsistido(checked); if (checked) setAssistedDraft({ ...assistedDraft, persona_autorizada_1: { nombre: [assistedDraft.representante_nombres, assistedDraft.representante_apellido_paterno, assistedDraft.representante_apellido_materno].filter(Boolean).join(' '), dni: assistedDraft.representante_dni, parentesco: assistedDraft.vinculo_representante || 'Representante legal', celular: assistedDraft.celular } }); }} className="h-4 w-4 accent-primary-700"/><span>El representante legal será la persona autorizada principal</span></label>{[1,2].map((number) => { const key = `persona_autorizada_${number}`; const person = assistedDraft[key] || {}; return <div key={key} className="mt-3 rounded-lg border bg-white p-3"><p className="mb-2 text-sm font-semibold">{number === 1 ? 'Persona principal *' : 'Persona alternativa (opcional)'}</p><div className="grid gap-2 md:grid-cols-4">{[['nombre','Nombre completo'],['dni','DNI'],['parentesco','Parentesco'],['celular','Celular']].map(([property,label]) => <input key={property} value={person[property] || ''} onChange={(e) => setAssistedDraft({ ...assistedDraft, [key]: { ...person, [property]: property === 'dni' ? e.target.value.replace(/\D/g, '').slice(0, 8) : e.target.value } })} inputMode={property === 'dni' ? 'numeric' : undefined} maxLength={property === 'dni' ? 8 : undefined} placeholder={`${label}${number === 1 ? ' *' : ''}`} className={field}/>)}</div></div>; })}</div><div className="flex justify-end"><button type="button" onClick={saveAssistedDraft} disabled={assistedSaving} className={`${btn} bg-primary-700 text-white`}>{assistedSaving ? 'Guardando...' : 'Guardar borrador para verificar'}</button></div></div>}</div><div className={`mt-6 rounded-xl border p-4 ${detail.aceptado_en ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-blue-200 bg-blue-50 text-blue-800'}`}><b>{detail.aceptado_en ? 'Datos aceptados por el apoderado' : 'Vista previa del borrador'}</b><p className="mt-1 text-sm">{detail.aceptado_en ? 'Esta información forma parte de la evidencia aceptada de la matrícula.' : 'Estos datos fueron preparados por el colegio y aún deben ser revisados y aceptados por el apoderado.'}</p></div><h3 className="mt-6 font-semibold text-primary-800">Datos del/de la estudiante</h3><div className="mt-2 grid gap-3 md:grid-cols-2"><div className="rounded-lg border p-3 text-sm"><p><b>Apellidos y nombres:</b> {[detailData?.alumno_apellido_paterno, detailData?.alumno_apellido_materno, detailData?.alumno_nombres].filter(Boolean).join(' ') || detail.datos_snapshot?.alumno?.nombre || '-'}</p><p><b>DNI:</b> {detailData?.alumno_dni || detail.datos_snapshot?.alumno?.dni || '-'}</p><p><b>Nacimiento:</b> {detailData?.alumno_fecha_nacimiento || '-'}</p><p><b>Sexo:</b> {detailData?.alumno_sexo || '-'}</p></div><div className="rounded-lg border p-3 text-sm"><p><b>Origen:</b> {[detailData?.alumno_pais_origen, detailData?.alumno_ubicacion_origen].filter(Boolean).join(' · ') || '-'}</p><p><b>Cuidado especial:</b> {detailData?.alumno_requiere_cuidado_especial === 'SI' ? detailData?.alumno_detalle_cuidado_especial || 'Sí' : 'No'}</p><p><b>Grado de matrícula:</b> {detail.datos_snapshot?.alumno?.nivel} · {detail.datos_snapshot?.alumno?.grado} {detail.datos_snapshot?.alumno?.seccion}</p></div></div>{detail.aceptado_en && <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-blue-200 bg-blue-50 p-3 print:hidden"><p className="text-sm text-blue-800">Si verificó las correcciones, puede trasladar el nombre y DNI aceptados a la ficha principal. La evidencia histórica permanecerá intacta.</p><button type="button" onClick={syncStudentRecord} disabled={studentSyncing} className={`${btn} bg-blue-700 text-white`}>{studentSyncing ? 'Actualizando...' : 'Actualizar ficha del alumno'}</button></div>}<h3 className="mt-6 font-semibold text-primary-800">Antecedente y situación escolar</h3><div className="mt-2 grid gap-3 md:grid-cols-2"><div className="rounded-lg border p-3 text-sm"><p><b>Tipo de ingreso:</b> {detailData?.tipo_ingreso?.replaceAll('_',' ') || 'No registrado'}</p><p><b>Condición:</b> {detailData?.condicion_promocion?.replaceAll('_',' ') || 'No aplica'}</p><p><b>Grado de matrícula:</b> {detail.datos_snapshot?.alumno?.nivel} · {detail.datos_snapshot?.alumno?.grado} {detail.datos_snapshot?.alumno?.seccion}</p></div><div className="rounded-lg border p-3 text-sm"><p><b>Antecedente:</b> {detailData?.anio_escolar_anterior || '-'} · {detailData?.nivel_anterior || ''} {detailData?.grado_anterior || ''}</p><p><b>Procedencia:</b> {detailData?.institucion_procedencia || '-'}</p><p><b>Código modular:</b> {detailData?.codigo_modular_procedencia || '-'}</p><p><b>Ubicación:</b> {detailData?.ubicacion_procedencia || '-'}</p></div></div><h3 className="mt-6 font-semibold text-primary-800">Autorización de salida</h3><div className={`mt-2 rounded-lg border p-4 text-sm font-semibold ${detailData?.autoriza_salida_solo === 'SI' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : detailData?.autoriza_salida_solo === 'NO' ? 'border-amber-200 bg-amber-50 text-amber-800' : 'border-gray-200 bg-gray-50 text-gray-600'}`}>{detailData?.autoriza_salida_solo === 'SI' ? 'Sí: el apoderado autoriza que el alumno(a) salga solo(a) del colegio.' : detailData?.autoriza_salida_solo === 'NO' ? 'No: el alumno(a) debe ser recogido(a) por una persona autorizada.' : 'Pendiente de decisión del apoderado.'}</div><h3 className="mt-6 font-semibold text-primary-800">Personas autorizadas en una emergencia</h3><div className="mt-2 grid gap-2 md:grid-cols-2">{[detailData?.persona_autorizada_1, detailData?.persona_autorizada_2].filter((p) => p?.nombre).map((p, i) => <div key={i} className="rounded-lg border p-3 text-sm"><b>{p.nombre}</b><p>DNI: {p.dni} · {p.parentesco}</p><p>Celular: {p.celular}</p></div>)}</div><h3 className="mt-6 font-semibold text-primary-800">Control de entrega documental</h3><p className="mt-1 text-sm text-primary-500">Este control solo registra la entrega física; no almacena archivos.</p><div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold"><span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-3 py-1.5 text-emerald-800"><HiCheckCircle className="h-4 w-4"/>{documentSummary.ENTREGADO} entregados</span><span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1.5 text-amber-800"><HiClock className="h-4 w-4"/>{documentSummary.PENDIENTE} pendientes</span><span className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-3 py-1.5 text-gray-600"><HiMinusCircle className="h-4 w-4"/>{documentSummary.NO_APLICA} no aplica</span></div><div className="mt-3 space-y-2">{documentosControl.map(([key, label]) => { const item = documentCheck[key] || { estado: 'PENDIENTE', observacion: '' }; const visual = documentStatusClasses[item.estado] || documentStatusClasses.PENDIENTE; const StatusIcon = visual.icon; return <div key={key} className={`grid gap-2 rounded-lg border p-3 transition-colors md:grid-cols-[1fr_170px_1fr] md:items-center ${visual.row}`}><span className="inline-flex items-center gap-2 text-sm font-medium"><StatusIcon className={`h-5 w-5 flex-none ${visual.iconClass}`}/>{label}</span><select value={item.estado} onChange={(e) => setDocumentCheck({ ...documentCheck, [key]: { ...item, estado: e.target.value } })} className={`${field} ${visual.select}`}><option value="PENDIENTE">Pendiente</option><option value="ENTREGADO">Entregado</option><option value="NO_APLICA">No aplica</option></select><input value={item.observacion || ''} onChange={(e) => setDocumentCheck({ ...documentCheck, [key]: { ...item, observacion: e.target.value } })} placeholder="Observación opcional" maxLength="500" className={field}/></div>; })}</div><div className="mt-3 flex justify-end print:hidden"><button type="button" onClick={saveDocumentCheck} disabled={documentSaving} className={`${btn} bg-primary-700 text-white`}>{documentSaving ? 'Guardando...' : 'Guardar control de entrega'}</button></div><h3 className="mt-6 font-semibold text-primary-800">Aceptaciones</h3><div className="mt-2 grid gap-2 md:grid-cols-2">{detail.documentos_snapshot.map((d) => <div key={d.clave} className="flex items-center gap-2 rounded-lg border p-3"><HiCheckCircle className={detail.aceptaciones_json?.[d.clave] ? 'text-emerald-600' : 'text-gray-300'} />{d.nombre} · versión {d.version}</div>)}</div><h3 className="mt-6 font-semibold text-primary-800">Trazabilidad</h3><div className="mt-2 space-y-2">{detail.eventos.map((e) => <div key={e.id} className="flex justify-between rounded-lg bg-cream-50 p-3 text-sm"><span>{String(e.evento).replaceAll('_',' ')}</span><span className="text-primary-500">{new Date(e.creado_en).toLocaleString('es-PE')}</span></div>)}</div>{detail.hash_evidencia && <p className="mt-5 break-all rounded-lg border border-dashed border-gold-400 p-3 font-mono text-xs">Huella de evidencia SHA-256: {detail.hash_evidencia}</p>}<div className="mt-6 print:hidden"><textarea value={observation} onChange={(e) => setObservation(e.target.value)} placeholder="Observación de revisión" className={field}/><div className="mt-3 flex flex-wrap justify-end gap-2"><button onClick={() => window.print()} className={`${btn} border`}><HiPrinter/>Imprimir evidencia</button><button onClick={() => review('OBSERVADA')} disabled={!detail.aceptado_en || !observation.trim()} className={`${btn} bg-amber-500 text-white`}>Observar</button><button onClick={() => review('COMPLETADA')} disabled={!detail.aceptado_en} className={`${btn} bg-emerald-600 text-white`}>Completar matrícula</button></div></div></div></div>}
  </div>;
}
