  ...
  const saveAssistedDraft = async () => {
    ...
    try {
      if (detail.estado === 'COMPLETADA') {
        ...
        const response = await guardarComplementoAdministrativoMatricula(detail.id, assistedDraft, adminComplementReason);
        setDetail({ ...detail, ...response.data.data, complemento_administrativo: response.data.data.complemento_administrativo });
        setAssistedEditing(false); setAdminComplementEditing(false); setAdminComplementReason(''); await load();
