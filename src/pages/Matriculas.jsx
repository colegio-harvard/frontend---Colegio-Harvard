...
export default function Matriculas() {
  ...
  return <div className="space-y-6 pb-10">
    ...
    {detail?.estado === 'COMPLETADA' && usuario?.rol_codigo === 'SUPER_ADMIN' && <div className="fixed bottom-5 right-5 z-[80] max-w-sm rounded-xl border border-blue-300 bg-blue-50 p-4 shadow-xl print:hidden"><p className="text-sm font-semibold text-blue-900">Complemento administrativo</p><p className="mt-1 text-xs text-blue-800">Permite completar datos sin alterar la aceptación ni la huella de evidencia original.</p>{adminComplementEditing && <textarea value={adminComplementReason} onChange={(e) => setAdminComplementReason(e.target.value)} placeholder="Justificación obligatoria (mínimo 10 caracteres)" maxLength="1000" className={`${field} mt-3 bg-white text-sm`}/>}<button type="button" onClick={toggleAdminComplement} className={`${btn} mt-3 w-full bg-blue-700 text-white`}>{adminComplementEditing ? 'Cerrar complemento' : (detail.complementado_en ? 'Editar complemento administrativo' : 'Completar ficha administrativa')}</button>{detail.complementado_en && !adminComplementEditing && <p className="mt-2 text-xs text-blue-700">Última actualización: {new Date(detail.complementado_en).toLocaleString('es-PE')}<br/>Motivo: {detail.motivo_complemento}</p>}</div>}
