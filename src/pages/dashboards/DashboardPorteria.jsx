...
const PensionResumen = ({ pensiones }) => {
  ...
  return (
    <div className="w-full mt-4 pt-4 border-t border-cream-200">
  ...
  return (
    <div className={`mt-4 w-full animate-pulse rounded-xl border-2 p-4 text-left shadow-sm motion-reduce:animate-none ${urgente ? 'border-red-500 bg-red-50' : 'border-amber-400 bg-amber-50'}`} role="alert" aria-live="assertive">
      <div className="flex gap-3"><HiExclamation className={`h-7 w-7 flex-shrink-0 ${urgente ? 'text-red-600' : 'text-amber-600'}`} /><div className="min-w-0 flex-1">
        ...
        {alerta.ultima_accion && <p className="mt-3 rounded-md bg-white/70 px-2 py-1.5 text-[11px] font-semibold text-primary-800">{alerta.ultima_accion === 'DERIVAR_OFICINA' ? 'Pendiente: recepción del alumno en oficina.' : 'Pendiente: atención del aviso por administración.'}</p>}
      </div></div>
    </div>
  );
};
