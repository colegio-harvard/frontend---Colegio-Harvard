...
function imprimirLibreta(data, ventana, periodoImpreso = periodoSeleccionadoParaImpresion) {
  ...
  const fraseInstitucional = data.fraseInstitucional || '24 años formando generaciones, más de 2800 estudiantes y miles de historias que nos inspiran a seguir creciendo juntos.';
  ...
  const iconosPadre = ['⌂','♡','☎','▤','☆','◷','♧'];
  const filasPadre = conceptosPadre.map((concepto, i) => `<tr><td><span class="parent-icon">${iconosPadre[i] || '◇'}</span><span>${concepto}</span></td>${[1,2,3,4].map(p => `<td>${nota(notasPadre,concepto,p,'nombre')}</td>`).join('')}</tr>`).join('');
...
export default function Libretas() {
  ...
  const personalizarFrase = async () => { const frase=window.prompt('Frase institucional del año',data?.fraseInstitucional||''); if(frase===null)return; if(!frase.trim())return toast.error('Ingrese una frase institucional'); try{await api.guardarFraseInstitucional(frase.trim());await recargar();toast.success('Frase institucional actualizada');}catch(e){toast.error(errorText(e));} };
  return <div className="p-4 md:p-7 space-y-5"><div className="bg-white border border-cream-200 rounded-2xl p-5 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4"><div><h1 className="text-2xl text-primary-800">Libretas y evaluación académica</h1><p className="text-sm text-cream-600 mt-1">Año escolar {data?.anio.anio} · Calificación AD, A, B y C</p></div>{admin&&<button className="btn-gold whitespace-nowrap" onClick={personalizarFrase}><HiPencil className="inline mr-2"/>Personalizar frase institucional</button>}</div>
    ...
    {tab==='acom'&&<section className="bg-white p-5 rounded-2xl border space-y-5">
      <div><h2 className="text-lg font-semibold text-primary-800">Conducta, tutoría y nota del padre</h2><p className="text-sm text-cream-600">Seleccione el aula, bimestre y alumno. Estas calificaciones no intervienen en el orden de mérito.</p></div>
      <div className={`grid gap-3 ${admin?'md:grid-cols-3':'md:grid-cols-2'}`}>
        {admin&&<label className="text-sm">Responsable<select className="input-field mt-1" value={docenteFiltro} onChange={e=>setDocenteFiltro(e.target.value)}><option value="TODOS">Todos los responsables</option>{responsables.map(r=><option key={r.id} value={r.id}>{r.nombre}</option>)}</select></label>}
        <label className="text-sm">Aula<select className="input-field mt-1" value={seleccion?.id_aula||''} onChange={e=>{const aula=aulasConducta.find(a=>String(a.id_aula)===e.target.value);setAsigId(aula?String(aula.id):'');setAcom({alumno:'',conducta:{},padreNotas:{},tutor:'',padre:''});}}><option value="">Seleccione aula</option>{aulasConducta.map(a=><option key={a.id_aula} value={a.id_aula}>{a.grado} {a.seccion} · {a.nivel}</option>)}</select></label>
        <label className="text-sm">Bimestre<select className="input-field mt-1" value={periodoId} onChange={e=>{setPeriodoId(e.target.value);setAcom({alumno:'',conducta:{},padreNotas:{},tutor:'',padre:''});}}>{data?.periodos.map(p=><option key={p.id} value={p.id}>{p.nombre} · {p.estado}</option>)}</select></label>
      </div>
      <select className="input-field" value={acom.alumno} onChange={e=>seleccionarAlumnoConducta(e.target.value)}><option value="">Seleccione alumno</option>{gradebook?.alumnos.map(a=><option key={a.id} value={a.id}>{a.codigo_alumno} · {a.nombre_completo}</option>)}</select>
      <div><h3 className="font-semibold text-primary-800 mb-3">Conducta y hábitos</h3><div className="grid md:grid-cols-2 gap-3">{data?.criterios.map(c=><label key={c.id} className="flex justify-between items-center gap-3 border border-cream-200 rounded-xl p-3"><span>{c.nombre}</span><select className="input-field !w-24" value={acom.conducta[c.id]||''} onChange={e=>setAcom({...acom,conducta:{...acom.conducta,[c.id]:e.target.value}})}>{NOTAS.map(n=><option key={n} value={n}>{n||'—'}</option>)}</select></label>)}</div></div>
      <div className="border-t border-cream-200 pt-4"><h3 className="font-semibold text-primary-800 mb-3">Comentario del tutor</h3><select className="input-field" value={acom.tutor} onChange={e=>setAcom({...acom,tutor:e.target.value})}><option value="">Seleccione un comentario (opcional)</option>{commentsTutor.map(c=><option key={c.id} value={c.id}>{c.texto}</option>)}</select></div>
      <div className="border-t border-cream-200 pt-4"><h3 className="font-semibold text-primary-800">Nota del padre de familia</h3><p className="text-sm text-cream-600 mb-3">Registre AD, A, B o C para cada concepto.</p><div className="grid md:grid-cols-2 gap-3">{data?.criteriosPadre?.map(c=><label key={c.id} className="flex justify-between items-center gap-3 border border-cream-200 rounded-xl p-3"><span>{c.nombre}</span><select className="input-field !w-24" value={acom.padreNotas?.[c.id]||''} onChange={e=>setAcom({...acom,padreNotas:{...(acom.padreNotas||{}),[c.id]:e.target.value}})}>{NOTAS.map(n=><option key={n} value={n}>{n||'—'}</option>)}</select></label>)}</div></div>
      <div className="flex justify-end"><button className="btn-primary" disabled={!acom.alumno||(!admin&&periodo?.estado!=='ABIERTO')} onClick={guardarConducta}>Guardar conducta, tutoría y nota del padre</button></div>
    </section>}
  </div>;
}
