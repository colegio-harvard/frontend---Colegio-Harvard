...
const tabsAdmin = [
  ["registro", "Registro de notas", HiBookOpen],
  ["acom", "Conducta y hábitos", HiClipboardCheck],
  ["config", "Cursos y responsables", HiCog],
  ["periodos", "Bimestres", HiClipboardCheck],
  ["merito", "Orden de mérito", HiAcademicCap],
  ...
  return (
    <div className="relative mt-3">
      ...
      >
        <span>
          {ids.length
            ? `${ids.length} curso${ids.length === 1 ? "" : "s"} seleccionado${ids.length === 1 ? "" : "s"}`
      ...
      {abierto && (
        <div className="absolute z-30 mt-1 w-full max-h-72 overflow-y-auto rounded-xl border border-gold-300 bg-white p-2 shadow-xl">
          <div className="flex justify-between px-2 py-1 text-xs">
            <button
              type="button"
              className="text-gold-700 font-medium"
              onClick={() => onChange(cursos.map((c) => Number(c.id)))}
            >
              Seleccionar todos
            </button>
            <button
          ...
          {cursos.map((c) => (
            <label
              key={c.id}
              className="flex items-center gap-3 rounded-lg px-3 py-2 hover:bg-cream-50 cursor-pointer"
            >
              <input
                type="checkbox"
                className="w-4 h-4 accent-red-700"
                checked={ids.includes(Number(c.id))}
                onChange={() => toggle(Number(c.id))}
              />
              <span className="text-sm">
                <b>{c.nombre}</b>
                <small className="block text-cream-600">{c.area}</small>
              </span>
            </label>
          ))}
          <button
...
function imprimirLibretaAnterior(data, ventana) {
  if (!ventana) {
    ...
    if (!ventana)
      return toast.error("Permita las ventanas emergentes para imprimir");
    ventana.opener = null;
  }
  ...
  const cursos = [
    ...new Map(
      notas.map((n) => [
        `${n.area}|${n.curso}`,
        { area: n.area, curso: n.curso },
      ]),
    ).values(),
  ];
  const celda = (arr, nombre, p, campo = "curso") =>
    arr.find((x) => x[campo] === nombre && Number(x.numero) === p)
      ?.calificacion || "";
  const comentario = (p) =>
    observaciones.find(
      (x) => x.tipo === "COMENTARIO_TUTOR" && Number(x.numero) === p,
    )?.texto ||
    observaciones.find(
      (x) => x.tipo === "COMENTARIO_DOCENTE" && Number(x.numero) === p,
    )?.texto ||
    "";
...
function imprimirLibreta(
  data,
  ventana,
  periodoImpreso = periodoSeleccionadoParaImpresion,
) {
  if (!ventana) {
    ventana = window.open("", "_blank", "width=1200,height=850");
    if (!ventana)
  ...
  const nombrePeriodoImpreso = ["I", "II", "III", "IV"][
    numeroPeriodoImpreso - 1
  ];
  const nota = (lista, nombre, periodo, campo = "curso") =>
    esc(
      lista.find((x) => x[campo] === nombre && Number(x.numero) === periodo)
        ?.calificacion || "",
    );
  const notaNumerica = (nombre, periodo) => {
    const valor = notas.find(
      (x) => x.curso === nombre && Number(x.numero) === periodo,
    )?.nota_numerica;
    return valor === null || valor === undefined ? "" : Number(valor);
  };
  const letraNumerica = (valor) =>
    valor === ""
      ? ""
      : Number(valor) >= 16
        ? "AD"
  ...
  const grupos = cursos.reduce(
    (m, c) => ((m[c.area] = [...(m[c.area] || []), c]), m),
    {},
  );
  const ordenAreasPrimaria = [
    "MATEMÁTICA",
    "COMUNICACIÓN",
    "INGLÉS",
    "PERSONAL SOCIAL",
  ...
  const comentariosCurso = (curso) =>
    esc(
      observaciones.find(
        (x) =>
          x.tipo === "COMENTARIO_DOCENTE" &&
          x.curso === curso &&
          Number(x.numero) === numeroPeriodoImpreso,
      )?.texto || "",
    );
  ...
  const filasCursosInicial = gruposOrdenados
    .map(([area, lista]) =>
      lista
        .map(
          (c, i) =>
            `<tr>${i === 0 ? `<td class="area" rowspan="${lista.length}"><span class="row-icon area-icon">${iconoArea(area)}</span><span>${esc(area)}</span></td>` : ""}<td class="course-name">${esc(c.curso)}</td>${[1, 2, 3, 4].map((p) => `<td class="grade">${nota(notas, c.curso, p)}</td>`).join("")}<td class="course-comment">${comentariosCurso(c.curso)}</td></tr>`,
        )
        .join(""),
    )
    .join("");
  const filasCursosPrimaria = gruposOrdenados
    .map(([area, lista]) =>
      lista
        .map((c, i) => {
          const valores = [1, 2, 3, 4].map((p) => notaNumerica(c.curso, p));
          const registrados = valores.filter((v) => v !== "");
          const promedio = registrados.length
            ? registrados.reduce((s, v) => s + Number(v), 0) /
              registrados.length
            : "";
          const celdas = valores
            .map(
              (v) =>
                `<td class="numeric-grade ${v !== "" && Number(v) <= 10 ? "low" : "high"}">${v === "" ? "" : String(v).padStart(2, "0")}</td><td class="letter-grade">${letraNumerica(v)}</td>`,
            )
            .join("");
          const promedioTexto =
            promedio === "" ? "" : promedio.toFixed(2).replace(/\.00$/, "");
          return `<tr>${i === 0 ? `<td class="area" rowspan="${lista.length}"><span class="row-icon area-icon">${iconoArea(area)}</span><span>${esc(area)}</span></td>` : ""}<td class="course-name">${esc(c.curso)}</td>${celdas}<td class="numeric-grade ${promedio !== "" && promedio <= 10 ? "low" : "high"}">${promedioTexto}</td><td class="letter-grade">${letraNumerica(promedio)}</td></tr>`;
        })
        .join(""),
    )
    .join("");
  ...
  const filasConducta = nombresConducta
    .map(
      (n, i) =>
        `<tr><td><span class="row-icon conduct-icon">${iconosConducta[i] || "◇"}</span><span>${esc(n)}</span></td>${[1, 2, 3, 4].map((p) => `<td class="grade">${nota(conducta, n, p, "nombre")}</td>`).join("")}</tr>`,
    )
    .join("");
  const observacion = (tipo, p) =>
    esc(
      observaciones.find((x) => x.tipo === tipo && Number(x.numero) === p)
        ?.texto || "",
    );
  const foto = alumno.foto_url ? fileUrl(alumno.foto_url) : "";
  const fraseInstitucional =
    data.fraseInstitucional ||
    "24 años formando generaciones, más de 2800 estudiantes y miles de historias que nos inspiran a seguir creciendo juntos.";
  const partesNombre = String(alumno.nombre_completo || "")
    .trim()
    .split(/\s+/)
    .filter(Boolean);
  const nombresAlumno =
  ...
  <section class="sheet"><div class="spread"><div class="panel">
    ...
    <div class="legend">${[
      [
        ...
        "#1499a8",
        "LOGRO DESTACADO",
        "Supera lo esperado respecto a la competencia.",
      ],
      [
        "A",
        "#4ca564",
        "LOGRO ESPERADO",
        "Alcanza satisfactoriamente lo programado.",
      ],
      ["B", "#e29a17", "EN PROCESO", "Requiere acompañamiento para lograrlo."],
      ["C", "#d9343a", "EN INICIO", "Necesita mayor tiempo y apoyo docente."],
    ]
      .map(
        (x) =>
          `<div class="legend-row"><div class="legend-key" style="background:${x[1]}">${x[0]}</div><div class="legend-text"><b>${x[2]}</b><br>${x[3]}</div></div>`,
      )
      .join(
        "",
      )}</div><div class="school-contact"><b>Horacio Zeballos, grupo D, manzana F, lote 6 – Ate</b><br>Teléfono / WhatsApp: 946-413-462 &nbsp; · &nbsp; E-mail: mbafjpg@gmail.com</div>
  ...
  const conceptosPadre = criteriosPadre.length
    ? criteriosPadre.map((c) => c.nombre)
    : [
        "Ayuda al niño en sus tareas",
        "Ayuda a corregir las malas conductas del niño",
        "Asiste a los llamados del profesor",
        "Cumple el Reglamento del Colegio",
        "Participa en las actividades",
        "Asiste a las reuniones",
        "Conducta del padre",
      ];
  const iconosPadre = ["⌂", "♡", "☎", "▤", "☆", "◷", "♧"];
  const filasPadre = conceptosPadre
    .map(
      (concepto, i) =>
        `<tr><td><span class="parent-icon">${iconosPadre[i] || "◇"}</span><span>${concepto}</span></td>${[1, 2, 3, 4].map((p) => `<td>${nota(notasPadre, concepto, p, "nombre")}</td>`).join("")}</tr>`,
    )
    .join("");
  const mejorasCaraUno = `<style>
    ...
    .sheet:first-of-type .scale{margin-top:20mm;margin-left:2.5mm;padding-left:3mm;border-left:1px solid #b9822f;font:2.9mm Georgia;line-height:1.85}.sheet:first-of-type .scale b{font-size:3.05mm}
    ...
    .sheet:first-of-type .parent-table td:first-child{white-space:nowrap;text-align:left}.sheet:first-of-type .parent-icon{display:inline-flex;width:5.5mm;height:4mm;align-items:center;justify-content:center;margin-right:1mm;color:#9b1723;font:3.4mm "Segoe UI Symbol",Georgia,serif;vertical-align:middle}.sheet:first-of-type .parent-table td:first-child>span:last-child{vertical-align:middle}
    ...
    .sheet:first-of-type .school-contact{position:absolute;left:8mm;right:8mm;bottom:4mm;padding-top:2mm;border-top:1px solid #d5a34c;text-align:center;color:#6f1519;font:2.25mm Georgia,serif;line-height:1.55;letter-spacing:.02mm}
    ...
    .sheet:first-of-type .spread>.panel.brand:after{content:'“Porque el Señor al que ama, disciplina” · Hebreos 12:6';position:absolute;right:0;top:0;width:9mm;height:100%;background:#741018;color:#fff;border-left:1mm solid #d3a348;writing-mode:vertical-rl;transform:rotate(180deg);display:flex;align-items:center;justify-content:center;font:700 italic 3.15mm Georgia,serif;letter-spacing:.15mm;text-shadow:0 0 .12mm #fff}
    ...
    .sheet:nth-of-type(2) .primary-grades{table-layout:fixed;font-size:2.15mm}
    .sheet:nth-of-type(2) .primary-grades th,.sheet:nth-of-type(2) .primary-grades td{padding:.55mm .35mm;height:5.65mm}
    .sheet:nth-of-type(2) .primary-grades th:nth-child(1){width:34mm}.sheet:nth-of-type(2) .primary-grades th:nth-child(2){width:45mm}
    .sheet:nth-of-type(2) .primary-grades th:nth-child(n+3){width:8.2mm}
    .sheet:nth-of-type(2) .primary-grades .area{width:34mm!important;white-space:normal}
    .sheet:nth-of-type(2) .primary-grades .area>span:last-child{width:calc(100% - 8mm);white-space:normal!important;font-size:2.05mm;line-height:1.1}
    .sheet:nth-of-type(2) .primary-grades .numeric-grade,.sheet:nth-of-type(2) .primary-grades .letter-grade{text-align:center;font-weight:700}
    .sheet:nth-of-type(2) .primary-grades .numeric-grade.high{color:#1257a6}.sheet:nth-of-type(2) .primary-grades .numeric-grade.low{color:#cf1826}
    ...
    .sheet.primary-sheet .comments{display:grid;grid-template-rows:auto repeat(4,minmax(0,1fr));height:100%}
    .sheet.primary-sheet .comment{min-height:0;padding:2mm;font-size:2.65mm}
  ...
  let htmlFinal = html
    .replace(
      /<tbody><tr><td>Acompañamiento y apoyo familiar<\/td>.*?<\/tr><\/tbody>/,
      `<tbody>${filasPadre}</tbody>`,
    )
    .replace(
      "Supera lo esperado respecto a la competencia.",
      "Cuando el estudiante evidencia un nivel superior a lo esperado respecto a la competencia. Demuestra aprendizajes que van más allá del nivel esperado.",
    )
    .replace(
      "Alcanza satisfactoriamente lo programado.",
      "Cuando el estudiante evidencia el nivel esperado respecto a la competencia, demostrando manejo satisfactorio en todas las áreas propuestas y en el tiempo programado.",
    )
    .replace(
      "Requiere acompañamiento para lograrlo.",
      "Cuando el estudiante está próximo o cerca al nivel esperado respecto a la competencia, requiere acompañamiento durante un tiempo razonable para lograrlo.",
    )
    .replace(
      "Necesita mayor tiempo y apoyo docente.",
      "Cuando el estudiante muestra un progreso mínimo y evidencia dificultades frecuentes, necesita mayor tiempo de acompañamiento e intervención del docente.",
    )
    .replace("</head>", `${mejorasCaraUno}</head>`);
  htmlFinal = htmlFinal.replace(
    /(<div class="cover-grid">.*?<img class="logo" src=")[^"]+/,
    `$1${insigniaJesus}`,
  );
  htmlFinal = htmlFinal.replace(
  ...
  htmlFinal = htmlFinal.replace(
    ...
    htmlFinal = htmlFinal.replace(
      "<th>ÁREAS CURRICULARES</th><th>CURSOS POR ÁREA</th><th>I</th><th>II</th><th>III</th><th>IV</th><th>COMENTARIO DEL CURSO · BIMESTRE " +
        ...
        "</th>",
      "<th>ÁREAS CURRICULARES</th><th>CURSOS POR ÁREA</th><th>Núm.<br>I</th><th>Letra<br>I</th><th>Núm.<br>II</th><th>Letra<br>II</th><th>Núm.<br>III</th><th>Letra<br>III</th><th>Núm.<br>IV</th><th>Letra<br>IV</th><th>PF<br>Núm.</th><th>PF<br>Letra</th>",
    );
  }
  htmlFinal = htmlFinal.replace(
    '<div class="footer">',
...
export default function Libretas() {
  ...
  const [merito, setMerito] = useState([]);
  ...
  const seleccion = useMemo(
    () => data?.asignaciones.find((a) => String(a.id) === String(asigId)),
    [data, asigId],
  );
  const aulaAsignacion = useMemo(
    () => data?.aulas?.find((a) => String(a.id) === String(asignacion.id_aula)),
    [data, asignacion.id_aula],
  );
  const cursosAsignables = useMemo(
    () =>
      (data?.cursos || []).filter(
        (c) => !aulaAsignacion || c.nivel === String(aulaAsignacion.nivel).toUpperCase(),
      ),
    [data, aulaAsignacion],
  );
  ...
  const responsables = useMemo(() => {
    ...
    (data?.asignaciones || []).forEach((a) =>
      unicos.set(String(a.id_docente), {
        id: String(a.id_docente),
        nombre: a.docente,
      }),
    );
    ...
    () =>
      docenteFiltro === "TODOS"
        ...
        : (data?.asignaciones || []).filter(
            (a) => String(a.id_docente) === docenteFiltro,
          ),
    [data, docenteFiltro],
  );
  ...
  const periodo = data?.periodos.find(
    (p) => String(p.id) === String(periodoId),
  );
  useEffect(() => {
    periodoSeleccionadoParaImpresion = Number(periodo?.numero) || 1;
  }, [periodo?.numero]);
  const cargar = async () => {
  ...
  const saveComment = async (alumno) => {
    if (!comentarios[alumno]) return;
    try {
      await api.guardarComentarioDocente({
        id_asignacion: Number(asigId),
        id_periodo: Number(periodoId),
        id_alumno: alumno,
  ...
  const guardarConducta = async () => {
    ...
    try {
      await api.guardarAcompanamiento({
        id_alumno: Number(acom.alumno),
        id_periodo: Number(periodoId),
        conducta: Object.entries(acom.conducta)
          .filter(([, v]) => v)
          .map(([id, v]) => ({ id_criterio: Number(id), calificacion: v })),
        nota_padre: Object.entries(acom.padreNotas || {})
          .filter(([, v]) => v)
          .map(([id, v]) => ({ id_criterio: Number(id), calificacion: v })),
        observaciones: [
          acom.tutor && {
            tipo: "COMENTARIO_TUTOR",
            id_catalogo: Number(acom.tutor),
          },
          acom.padre && { tipo: "NOTA_PADRE", id_catalogo: Number(acom.padre) },
        ].filter(Boolean),
      });
  ...
  const tabs = admin
    ? tabsAdmin
    : [
        ["registro", "Mis cursos y notas", HiBookOpen],
        ...(usuario?.rol_codigo === "TUTOR"
          ? [["acom", "Conducta y tutoría", HiClipboardCheck]]
          : []),
      ];
  if (busy && !data)
    return <div className="p-8">Cargando módulo académico...</div>;
  const commentsDoc =
      data?.catalogo.filter((x) => x.tipo === "COMENTARIO_DOCENTE") || [],
    commentsTutor =
  ...
  const retirarArea = async (item) => {
    if (
      !window.confirm(
        `¿Retirar el área “${item.nombre}”? Sus cursos dejarán de mostrarse, pero las notas históricas se conservarán.`,
      )
    )
      return;
    try {
      await api.eliminarArea(item.id);
      await recargar();
  ...
  const editarCurso = async (item) => {
    const nombre = window.prompt("Nuevo nombre del curso", item.nombre);
    if (!nombre?.trim() || nombre.trim() === item.nombre) return;
    try {
      await api.editarCurso(item.id, {
        nombre: nombre.trim(),
  ...
  const retirarCurso = async (item) => {
    if (
      !window.confirm(
        `¿Retirar el curso “${item.nombre}”? Las notas históricas se conservarán.`,
      )
    )
      return;
    try {
      await api.eliminarCurso(item.id);
      await recargar();
  ...
  const personalizarFrase = async () => {
    const frase = window.prompt(
      "Frase institucional del año",
      data?.fraseInstitucional || "",
    );
    if (frase === null) return;
    if (!frase.trim()) return toast.error("Ingrese una frase institucional");
    try {
      await api.guardarFraseInstitucional(frase.trim());
      await recargar();
      toast.success("Frase institucional actualizada");
    } catch (e) {
      toast.error(errorText(e));
    }
  };
  return (
    <div className="p-4 md:p-7 space-y-5">
      <div className="bg-white border border-cream-200 rounded-2xl p-5 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl text-primary-800">
            Libretas y evaluación académica
          </h1>
          <p className="text-sm text-cream-600 mt-1">
            Año escolar {data?.anio.anio} · Calificación AD, A, B y C
          </p>
        </div>
        {admin && (
          <button
            className="btn-gold whitespace-nowrap"
            onClick={personalizarFrase}
          >
            <HiPencil className="inline mr-2" />
            Personalizar frase institucional
          </button>
        )}
      </div>
      <div className="flex gap-2 overflow-x-auto">
        {tabs.map(([id, label, Icon]) => (
          <button
            key={id}
            onClick={() => setTab(id)}
            className={`px-4 py-2 rounded-xl whitespace-nowrap flex items-center gap-2 ${tab === id ? "bg-primary-700 text-white" : "bg-white border border-cream-200 text-primary-700"}`}
          >
            <Icon />
            {label}
          </button>
      ...
      {tab === "config" && admin && (
        <section className="bg-white p-5 rounded-2xl border">
          <h2 className="text-lg">Asignación rápida de varios cursos</h2>
          <p className="text-sm text-cream-600 mt-1">
            Elija el aula, marque todos los cursos del responsable y guarde una
            sola vez.
          </p>
          <div className="grid md:grid-cols-3 gap-3 items-start">
            <select
              className="input-field mt-3"
              value={asignacion.id_aula}
              onChange={(e) =>
                setAsignacion({ ...asignacion, id_aula: e.target.value })
              }
            >
              <option value="">Aula</option>
              {data?.aulas.map((x) => (
                <option key={x.id} value={x.id}>
                  {x.nivel} · {x.grado} {x.seccion}
                </option>
              ))}
            </select>
            <SelectorCursos
              cursos={cursosAsignables}
              seleccionados={asignacion.id_cursos}
              onChange={(ids) =>
                setAsignacion({ ...asignacion, id_cursos: ids })
              }
            />
            <select
              className="input-field mt-3"
              ...
              onChange={(e) =>
                setAsignacion({ ...asignacion, id_docente: e.target.value })
              }
            >
          ...
          <button
            ...
            onClick={async () => {
              try {
                const r = await api.asignarCurso(asignacion);
                setAsignacion({ ...asignacion, id_cursos: [] });
                recargar();
                toast.success(r.data.mensaje || "Cursos asignados");
              } catch (e) {
                toast.error(errorText(e));
              }
            }}
          >
            Asignar {asignacion.id_cursos.length || ""} curso
            {asignacion.id_cursos.length === 1 ? "" : "s"}
          </button>
        </section>
      )}
      {tab === "registro" && (
        <section className="bg-white rounded-2xl border border-cream-200 p-5 space-y-4">
          ...
          {!data?.asignaciones.length && (
            <p className="p-5 bg-amber-50 rounded-xl">
              Aún no tiene cursos asignados. El Superadministrador debe realizar
              la asignación.
            </p>
          )}
          {gradebook && (
            <>
                          ...
                          {esNumericaSeleccionada ? (
                            <div className="flex items-center justify-center gap-3">
                              <input
                                ...
                                max="20"
                                step="1"
                                className={`input-field w-24 text-center font-bold ${notas[a.id] !== "" && Number(notas[a.id]) <= 10 ? "text-red-600" : "text-blue-700"}`}
                                value={notas[a.id] ?? ""}
                                placeholder="00–20"
                                onChange={(e) =>
                                  setNotas({ ...notas, [a.id]: e.target.value })
                                }
                              />
                              <span className="inline-flex min-w-12 justify-center rounded-lg bg-cream-100 px-3 py-2 font-bold text-primary-800">
                                {letraNotaNumerica(notas[a.id])}
                              </span>
                            </div>
                          ) : (
                            <select
                              className="input-field min-w-20"
                              value={notas[a.id] || ""}
                              onChange={(e) =>
                                setNotas({ ...notas, [a.id]: e.target.value })
                              }
                        ...
                        {admin && (
                          <td>
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  const r = await api.cargarLibreta(a.id);
                                  imprimirLibreta(r.data.data);
                                } catch (e) {
                                  toast.error(errorText(e));
                                }
                              }}
              ...
              {admin && (
                <input
                  ...
                  placeholder="Motivo obligatorio si modifica una nota existente"
                  value={motivo}
              ...
              <div className="flex justify-end">
                <button
                  disabled={!admin && periodo?.estado !== "ABIERTO"}
      ...
      {tab === "config" && admin && (
        <section className="grid lg:grid-cols-3 gap-4">
          <div className="bg-white p-5 rounded-2xl border">
            <h2 className="text-lg">Nueva área</h2>
            <input
            ...
            <button
              ...
              onClick={async () => {
                try {
                  await api.crearArea({ nombre: area });
                  setArea("");
                  recargar();
          ...
          <div className="bg-white p-5 rounded-2xl border">
            <h2 className="text-lg">Nuevo curso</h2>
            <select
              className="input-field mt-3"
              value={curso.nivel}
            ...
            <select
              className="input-field mt-3"
              value={curso.id_area}
              onChange={(e) => setCurso({ ...curso, id_area: e.target.value })}
            >
            ...
            <button
              ...
              onClick={async () => {
                try {
                  await api.crearCurso(curso);
                  setCurso({ nombre: "", id_area: "", nivel: "INICIAL" });
                  recargar();
          ...
          <div className="bg-white p-5 rounded-2xl border">
            <h2 className="text-lg">Asignar responsable</h2>
            <select
              className="input-field mt-3"
              value={asignacion.id_aula}
              onChange={(e) =>
            ...
            <select
              ...
              onChange={(e) =>
                setAsignacion({ ...asignacion, id_curso: e.target.value })
              }
            >
              <option value="">Curso</option>
              {cursosAsignables.map((x) => (
                <option key={x.id} value={x.id}>
                  {x.area} · {x.nombre}
                </option>
              ))}
            ...
            <button
              ...
              onClick={async () => {
                try {
                  await api.asignarCurso(asignacion);
                  recargar();
          ...
          <div className="lg:col-span-3 grid md:grid-cols-2 gap-4">
            <div className="bg-white p-5 rounded-2xl border">
              <h2 className="text-lg mb-3">Áreas registradas</h2>
              <div className="space-y-2">
                {data?.areas.map((x) => (
                  <div
                    key={x.id}
                    className="flex items-center justify-between gap-3 p-3 rounded-xl bg-cream-50 border border-cream-200"
                  >
                    <span className="font-medium">{x.nombre}</span>
                    <div className="flex gap-2">
                      <button
                        title="Editar área"
                        className="p-2 rounded-lg text-gold-700 hover:bg-gold-100"
                        onClick={() => editarArea(x)}
                      >
                        <HiPencil />
                      </button>
                      <button
            ...
            <div className="bg-white p-5 rounded-2xl border">
              <h2 className="text-lg mb-3">Cursos registrados</h2>
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {data?.cursos.map((x) => (
                  ...
                  >
                    <span>
                      <b>{x.nombre}</b>
                      <small className="block text-cream-600">{x.area}</small>
                    </span>
                    <div className="flex gap-2">
                      <button
                        title="Editar curso"
                        className="p-2 rounded-lg text-gold-700 hover:bg-gold-100"
                        onClick={() => editarCurso(x)}
                      >
                        <HiPencil />
                      </button>
                      <button
      ...
      {tab === "periodos" && admin && (
        <section className="grid md:grid-cols-4 gap-4">
          {data?.periodos.map((p) => (
            <div key={p.id} className="bg-white p-5 rounded-2xl border">
              <h2>{p.nombre}</h2>
              ...
              <select
                ...
                onChange={async (e) => {
                  try {
                    await api.cambiarPeriodo(p.id, e.target.value);
                    recargar();
                    toast.success("Bimestre actualizado");
                  } catch (err) {
        ...
        <section className="bg-white p-5 rounded-2xl border">
          <div className="flex gap-3 mb-4">
            <button
              ...
              onClick={async () => {
                try {
                  const r = await api.cargarMerito({
                    id_aula: seleccion.id_aula,
                    id_periodo: periodoId || undefined,
                  });
                  setMerito(r.data.data);
                } catch (e) {
                  toast.error(errorText(e));
                }
              }}
          ...
          {merito.map((x) => (
            <div
              className="grid grid-cols-[70px_1fr_100px] border-b p-3"
              key={x.id}
            >
              <b>#{x.posicion}</b>
              <span>
                {x.codigo_alumno} · {x.nombre_completo}
              </span>
              <b>{x.puntaje.toFixed(2)}</b>
            </div>
          ))}
        </section>
      )}
      {tab === "auditoria" && admin && (
        <section className="bg-white p-5 rounded-2xl border">
          <button
            ...
            onClick={async () => {
              const r = await api.cargarAuditoriaNotas();
          ...
          <div className="space-y-2">
            {audit.map((x) => (
              <div key={x.id} className="p-3 border rounded-xl text-sm">
                <b>{x.alumno}</b> · {x.curso} · {x.periodo}:{" "}
                {x.calificacion_anterior || "Sin nota"} → {x.calificacion_nueva}
                <br />
                <span className="text-cream-600">
                  {x.usuario} · {new Date(x.fecha).toLocaleString()} ·{" "}
                  {x.motivo || "Registro inicial"}
                </span>
              </div>
            ))}
          </div>
      ...
      {tab === "acom" && (
        <section className="bg-white p-5 rounded-2xl border space-y-5">
          <div>
            <h2 className="text-lg font-semibold text-primary-800">
              Conducta, tutoría y nota del padre
            </h2>
            <p className="text-sm text-cream-600">
              Seleccione el aula, bimestre y alumno. Estas calificaciones no
              intervienen en el orden de mérito.
            </p>
          </div>
          <div
          ...
          >
            ...
            <label className="text-sm">
              ...
              <select
                className="input-field mt-1"
                value={seleccion?.id_aula || ""}
                onChange={(e) => {
                  const aula = aulasConducta.find(
                    (a) => String(a.id_aula) === e.target.value,
                  );
                  setAsigId(aula ? String(aula.id) : "");
                  setAcom({
                    alumno: "",
                    conducta: {},
                    padreNotas: {},
          ...
          <select
            className="input-field"
            value={acom.alumno}
            onChange={(e) => seleccionarAlumnoConducta(e.target.value)}
          >
            <option value="">Seleccione alumno</option>
            {gradebook?.alumnos.map((a) => (
              <option key={a.id} value={a.id}>
                {a.codigo_alumno} · {a.nombre_completo}
              </option>
            ))}
          </select>
          <div>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              {data?.criterios.map((c) => (
                <label
                  key={c.id}
                  className="flex justify-between items-center gap-3 border border-cream-200 rounded-xl p-3"
                >
                  <span>{c.nombre}</span>
                  <select
                    className="input-field !w-24"
                    value={acom.conducta[c.id] || ""}
                    onChange={(e) =>
                      setAcom({
                        ...acom,
                        conducta: { ...acom.conducta, [c.id]: e.target.value },
                      })
                    }
                  >
                    {NOTAS.map((n) => (
          ...
          <div className="border-t border-cream-200 pt-4">
            ...
            <p className="text-sm text-cream-600 mb-3">
              Registre AD, A, B o C para cada concepto.
            </p>
            <div className="grid md:grid-cols-2 gap-3">
              {data?.criteriosPadre?.map((c) => (
                <label
                  key={c.id}
                  className="flex justify-between items-center gap-3 border border-cream-200 rounded-xl p-3"
                >
          ...
          <div className="flex justify-end">
            <button
              className="btn-primary"
              disabled={
                !acom.alumno || (!admin && periodo?.estado !== "ABIERTO")
              }
              onClick={guardarConducta}
            >
              Guardar conducta, tutoría y nota del padre
            </button>
          </div>
        </section>
      )}
