...
import { HiArrowLeft, HiAcademicCap, HiUser, HiUserGroup, HiChevronRight, HiDocumentText } from 'react-icons/hi';
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate('/reportes-semanales')} className="p-2 text-primary-800/70 hover:bg-cream-100 rounded-lg transition-colors">
          <HiArrowLeft className="w-5 h-5" />
        </button>
        <div>
          <h1 className="text-xl font-bold text-primary-800 font-display">{aula.grado?.nombre} "{aula.seccion}"</h1>
          <p className="text-sm text-gold-600">{aula.grado?.nivel?.nombre} — Reportes Semanales</p>
        </div>
      </div>
        ...
        <Card>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-primary-800/70">{aula.grado?.nombre} - Sección {aula.seccion}</p>
            </div>
          </div>
        </Card>
        <Card>
      ...
      {alumnos.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {alumnos.map(alumno => (
            <button key={alumno.id} onClick={() => navigate(`/reportes-semanales/alumno/${alumno.id}`)} className="text-left group">
              <Card className="hover:shadow-lg hover:border-gold-300 transition-all duration-200 h-full">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1">
                    <HiDocumentText className="w-4 h-4 text-cream-300 group-hover:text-gold-500 transition-colors" />
